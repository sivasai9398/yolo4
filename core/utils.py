...
def draw_bbox(image, bboxes,count, classes=read_class_names(cfg.YOLO.CLASSES), allowed_classes=list(read_class_names(cfg.YOLO.CLASSES).values()), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        if class_name not in allowed_classes:
            ...
            cen1,cen2=int((coor[1]+coor[3])/2),int((coor[0]+coor[2])/2)
            #cv2.rectangle(image,)
            if cen2==250:
                count=count+1
                print(count)
            if cen2>235:
                ...
                #print(count)
                #cv2.putText(image,"C",(cen1,cen2),5,1,(0,255,0),3)
                #if cen2>=400:
                    #ncar=ncar+1
                #print(ncar,cen2)
